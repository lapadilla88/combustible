<?php
	...
	if(empty($_POST['fechai']) OR empty($_POST['fechaf'])){
		...
		$query = "SELECT mantenimiento.idmantenimiento, mantenimiento.numeromantenimiento, vehiculo.placa, tipomantenimiento.nombretipomantenimiento, mantenimiento.fechamantenimiento, mantenimiento.fechapmantenimiento, mantenimiento.kilometraje, mantenimiento.costo FROM mantenimiento  
					INNER JOIN vehiculo ON mantenimiento.idvehiculo = vehiculo.idvehiculo 
					INNER JOIN tipomantenimiento ON mantenimiento.idtipomantenimiento = tipomantenimiento.idtipomantenimiento 
					WHERE mantenimiento.fechamantenimiento BETWEEN '$fechai' AND '$fechaf' AND vehiculo.idvehiculo LIKE '%$idvehiculo%' AND tipomantenimiento.idtipomantenimiento LIKE '%$idtipomantenimiento%'AND mantenimiento.codcombustible='$codcombustible' ORDER BY mantenimiento.fechamantenimiento DESC";
	}
	...
	$pdf->Cell(25,6,utf8_decode('N° Mantenimie.'),1,0,'C',1);
	$pdf->Cell(20,6,utf8_decode('Vehículo.'),1,0,'C',1);
	$pdf->Cell(70,6,utf8_decode('Tipo Mantenimiento'),1,0,'C',1);
	...
	$pdf->SetFont('Arial','',7);
	$pdf->SetWidths(array(25,20,70,20,25,15,15));
	$pdf->SetAligns(array('C','C','L','C','C','R','R'));
	while($row = $resultado->fetch_assoc())
	{
		$pdf->Row(array(utf8_decode($row['numeromantenimiento']),utf8_decode($row['placa']),utf8_decode($row['nombretipomantenimiento']),utf8_decode($row['fechamantenimiento']),utf8_decode($row['fechapmantenimiento']),utf8_decode($row['kilometraje']),number_format(utf8_decode($row['costo']),2)));
	}
