<?php
	...
	if(empty($_POST['fechai']) OR empty($_POST['fechaf'])){
		...
		$query = "SELECT vale.idvale, vale.numerovale, vehiculo.placa, grifo.nombregrifo, vale.fecha, vale.galonaje, vale.precio, combustible.nombrecombustible, ( vale.galonaje * vale.precio)AS total FROM vale  
					...
					INNER JOIN conductor ON vale.idconductor = conductor.idconductor
					INNER JOIN combustible ON vehiculo.idcombustible = combustible.idcombustible
					WHERE vale.fecha BETWEEN '$fechai' AND '$fechaf' AND vehiculo.idvehiculo LIKE '%$idvehiculo%' AND grifo.idgrifo LIKE '%$idgrifo%' AND combustible.idcombustible LIKE '%$idcombustible%' AND vale.codcombustible='$codcombustible' ORDER BY vale.fecha DESC";
	}
	...
	if(empty($_POST['fechai']) OR empty($_POST['fechaf'])){
		...
		$pdf->Cell(190,6,utf8_decode('Reporte Del ' . $fechai . ' al ' . $fechaf),0,1,'L');
	}
	$pdf->Cell(190,6,utf8_decode(' '),0,1,'C');
	
	$pdf->SetFillColor(232,232,232);
	$pdf->SetFont('Arial','B',9);
	$pdf->Cell(25,6,utf8_decode('N° Vale'),1,0,'C',1);
	$pdf->Cell(20,6,utf8_decode('Vehículo.'),1,0,'C',1);
	$pdf->Cell(55,6,utf8_decode('Grifo'),1,0,'C',1);
	...
	$pdf->SetFont('Arial','',7);
	$pdf->SetWidths(array(25,20,55,20,25,15,15,15));
	$pdf->SetAligns(array('C','C','L','C','C','R','R','R'));
	while($row = $resultado->fetch_assoc())
	{
		$pdf->Row(array(utf8_decode($row['numerovale']),utf8_decode($row['placa']),utf8_decode($row['nombregrifo']),utf8_decode($row['fecha']),utf8_decode($row['nombrecombustible']),number_format(utf8_decode($row['galonaje']),2),number_format(utf8_decode($row['precio']),2),number_format(utf8_decode($row['total']),2)));
	}
	...
	if(empty($_POST['fechai']) OR empty($_POST['fechaf'])){
		...
		$query = "SELECT sum( vale.galonaje * vale.precio)AS total FROM vale  
					...
					INNER JOIN conductor ON vale.idconductor = conductor.idconductor
					INNER JOIN combustible ON vehiculo.idcombustible = combustible.idcombustible
					WHERE vale.fecha BETWEEN '$fechai' AND '$fechaf' AND vehiculo.idvehiculo LIKE '%$idvehiculo%'  ORDER BY vale.fecha DESC"; 
	}
